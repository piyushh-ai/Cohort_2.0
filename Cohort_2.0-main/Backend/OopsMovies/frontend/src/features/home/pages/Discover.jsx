...
const OVERSCAN = 10; // extra rows buffer above/below viewport
...
export default function Discover() {
  ...
  const { visibleMovies, paddingTop, paddingBottom, startIdx } = useMemo(() => {
    if (allMovies.length === 0) {
      return { visibleMovies: [], paddingTop: 0, paddingBottom: 0, startIdx: 0 };
    }
  ...
  const doFetch = useCallback(async (filters, pg, reset, myFilterKey) => {
    ...
    try {
      ...
      const res = await discoverMoviesApi(
        { genre: filters.genre, sort: filters.sort, year: filters.year,
  ...
  useEffect(() => {
    ...
    setAllMovies([]);
    doFetch(
      { genre: activeGenre, sort: activeSort, year: activeYear, ratingIdx: activeRating },
      1, true, key
    );
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !loadingRef.current && !initLoad && page < totalPages) {
          setPage(p => p + 1);
        }
      },
      ...
      <section className="disc-hero">
        {heroMovie?.backdrop_path && (
          <div
            ...
            style={{ backgroundImage: `url(https://image.tmdb.org/t/p/w1280${heroMovie.backdrop_path})` }}
          />
        )}
        ...
        {heroMovie ? (
          <div className="disc-hero__body">
            <Link to={`/movie/${heroMovie.id}`} className="disc-hero__poster-wrap">
              <img
            ...
            <div className="disc-hero__info">
              ...
              <div className="disc-hero__meta">
                ...
                {heroMovie.release_date && (
                  <span className="disc-hero__year">{heroMovie.release_date.slice(0, 4)}</span>
                )}
                {heroMovie.vote_count && (
                  <span className="disc-hero__votes">{(heroMovie.vote_count / 1000).toFixed(0)}K votes</span>
                )}
              </div>
          ...
          <div className="disc-genre-scroll">
            {GENRES.map(g => (
              <button
                ...
                style={activeGenre === g.id
                  ? { "--c1": GENRE_GRADIENTS[g.id]?.[0], "--c2": GENRE_GRADIENTS[g.id]?.[1] }
                  : {}}
        ...
        <div className="disc-ctrl-bar">
          <div className="disc-field">
            ...
            <div className="disc-sel-wrap">
              ...
              <svg className="disc-sel__arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
          ...
          {isFiltered && (
            <button className="disc-reset-btn" onClick={() => {
              setActiveGenre("");
      ...
      <section className="disc-section">
        ...
        {!initLoad && !error && allMovies.length > 0 && (
          <div className="disc-meta">
            ...
            {activeGenreData?.id && (
              <span className="disc-meta__pill">{activeGenreData.emoji} {activeGenreData.name}</span>
            )}
        ...
        {initLoad && (
          <div className="disc-grid">
            {Array.from({ length: 20 }).map((_, i) => (
              <div key={i} className="disc-skel" style={{ animationDelay: `${i * 0.03}s` }} />
            ))}
          </div>
        )}
        ...
        {!initLoad && error && (
          <div className="disc-empty">
            ...
            <button className="disc-empty__btn"
              onClick={() => {
                const key = filterKeyRef.current;
